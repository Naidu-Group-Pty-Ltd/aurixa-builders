...
export interface UploadTelemetry {
  ...
  deterministic?: {
    ...
    readBy?: string[] | null;
    /**
     * What the property page's pictures came to, where one was read for a
     * building size the text did not state: `read` and its proofs, `refused`
     * and a reason per picture, or `unavailable`. Words only.
     */
    figures?: string[] | null;
...
export function uploadTelemetry(input: UploadTelemetry): TelemetryRecord {
  return compact({
    ...
    deterministic_figures: names(input.deterministic?.figures),
