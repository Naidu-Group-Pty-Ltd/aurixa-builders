/*
 * A HELD-BACK PATCH SAYS WHAT IT MEANS, INCLUDING WHEN IT MEANS NOTHING.
 *
 * `publish_builder_stock_upload` applied a deferred patch with
 * `coalesce(pending_patch->>'col', i.col)` on every column, which reads an
 * explicit null as "the patch is silent about this" and keeps the stored
 * value. That was inert while nothing could put a null in a patch, and it
 * stops being inert today: `writablePatch` now clears the columns a re-read
 * of the SAME FILE no longer states, so that a parser correction can reach a
 * row that already holds what it corrects.
 *
 * Applied to a published row, `coalesce` would have kept the old figure —
 * so the direct update would clear `land_size_sqm` and the deferred one
 * would not, and which behaviour a builder got would depend on whether their
 * property happened to be live at the time. Half of a contract deployed is
 * worse than neither half.
 *
 * KEY PRESENCE, NOT NULL-NESS. `pending_patch ? 'col'` is strictly true or
 * false, and it is the only thing that tells "stated as nothing" apart from
 * "not stated" — the same distinction the `manual_stats` CHECK constraint had
 * to learn, where `->` on an absent key is SQL NULL and a whole `and` chain
 * evaluated to NULL and was accepted.
 *
 * ON EVERY COLUMN, so this function applies the patch it is handed and
 * decides nothing. WHICH columns may be unsaid is `UNSAYABLE_ON_REREAD` in
 * `importStock.ts` and is named once: the identity columns a row is FOUND by
 * keep the skip there and so can never arrive here as a null, and two
 * implementations of that rule is how the two would drift.
 *
 * Nothing else about the function changes. The numeric casts already yield
 * NULL for a JSON null, `source_row` keeps its own defensive read, and the
 * promote/archive/telemetry steps are untouched.
 */
