 ...
 */
const CLASSES: Array<{ key: string; fixture: string; what: string }> = [
...
async function once(entry: Entry, iteration: number) {
  ...
  const storagePath = `${org.id}/latency-${crypto.randomUUID()}.pdf`;
  ...
  return {
    ok: true as const, importMs, totalMs, hops, published, terminal,
...
for (const cls of CLASSES) {
  const entry = manifest.find((e) => e.name === cls.fixture);
  if (!entry) {
  ...
  for (let n = 0; n < ITERATIONS; n += 1) {
    ...
    hops = run.hops; properties = run.properties;
  ...
  console.log(
    ...
    + `          hops ${hops}   properties ${properties}`
